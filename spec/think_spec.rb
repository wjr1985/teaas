...
RSpec.describe Teaas::Think do
  describe "#think" do
    let(:input) { TestImageFactory.create_animated_image(16, 16, 3) }
    let(:result) { Teaas::Think.think(input) }
    ...
    it "uses static_on_animated mode with SouthWestGravity" do
      expect(Teaas::Overlayer).to receive(:overlay).with(
        input,
        anything,
        hash_including(static_on_animated: true, gravity: Magick::SouthWestGravity)
      )
      Teaas::Think.think(input)
    end
  end

  it_behaves_like "a from_file wrapper (ImageList pattern)", Teaas::Think, :think, :think_from_file
end
